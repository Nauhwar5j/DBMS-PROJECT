...
document.addEventListener('DOMContentLoaded', () => {
                ...
                container.innerHTML = data.banks.map(b => `
                    <div class="bank-card" style="background:#fff; padding:20px; border-radius:18px; border:1px solid #e2e8f0; margin-bottom:12px; display:flex; justify-content:space-between; align-items:center;">
                        ...
                        <div style=\"text-align:right\">
                            <p id=\"balance-${b.bank_id || b.id}\" style=\"font-weight:800; color:#1e293b; margin-bottom:4px;\">₹ ••••••</p>
                            <button class=\"btn-glass\" onclick=\"checkBankBalance(${b.bank_id || b.id})\" style=\"font-size:11px; padding:4px 12px;\">Check Balance</button>
                        </div>
                    </div>
                `).join('');
            });
    ...
    window.simulateBankLink = function(bankName) {
        bankToLink = bankName;
        document.getElementById('confirm-bank-name').innerText = bankName;
        closeModals();
        const confirmModal = document.getElementById('modal-confirm-bank');
        if(confirmModal) confirmModal.classList.add('active');
    };

    document.getElementById('btn-confirm-link')?.addEventListener('click', () => {
        if(!bankToLink) return;
        showToast(`Linking ${bankToLink}...`, "info");
        fetch('/api/user/banks/link', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ bank_name: bankToLink })
        }).then(res => res.json()).then(data => {
            if(data.success) {
                showToast(`${bankToLink} Linked Successfully!`, "success");
                closeModals();
                loadLinkedBanks();
            } else {
                showToast(data.error || "Failed to link bank.", "error");
            }
        });
    });

    window.checkBankBalance = function(bankId) {
        const balEl = document.getElementById(`balance-${bankId}`);
        if(balEl) balEl.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
        fetch(`/api/user/banks/balance/${bankId}`)
            .then(res => res.json())
            .then(data => {
                if(data.success && balEl) {
                    balEl.innerText = `₹ ${parseFloat(data.balance).toLocaleString('en-IN', {minimumFractionDigits: 2})}`;
                } else {
                    showToast("Failed to fetch balance", "error");
                    if(balEl) balEl.innerText = "₹ Error";
                }
            });
    };
    ...
    function loadSupportTickets() {
        const container = document.getElementById('dynamic-ticket-list');
        if(!container) return;
        fetch('/api/user/complaints/list')
            .then(res => res.json())
            .then(data => {
                if(data.success && data.complaints && data.complaints.length > 0) {
                    container.innerHTML = data.complaints.map(c => `
                        <div style="background: #fff; border: 1px solid #e2e8f0; padding: 12px; border-radius: 12px; display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <div style="font-size: 11px; font-weight: 800; color: #64748b; text-transform: uppercase;">${c.category}</div>
                                <div style="font-size: 13px; font-weight: 700; color: var(--paytm-navy);">${c.ticket_id}</div>
                            </div>
                            <span style="font-size: 10px; font-weight: 800; padding: 4px 8px; border-radius: 6px; background: ${c.status === 'Open' ? '#fef08a' : '#dcfce7'}; color: ${c.status === 'Open' ? '#854d0e' : '#166534'}">${c.status.toUpperCase()}</span>
                        </div>
                    `).join('');
                } else {
                    container.innerHTML = '<div style="text-align: center; padding: 20px; font-size: 13px; color: #94a3b8;">No recent tickets</div>';
                }
            });
    }

    function resetHelpViews() {
        document.getElementById('help-list-view')?.classList.remove('hidden');
        document.getElementById('help-form-view')?.classList.add('hidden');
        document.getElementById('help-success-view')?.classList.add('hidden');
        const txt = document.getElementById('complaint-text');
        if(txt) txt.value = '';
    }
    ...
    document.getElementById('prof-menu-help')?.addEventListener('click', () => {
        loadSupportTickets();
        resetHelpViews();
    });
    ...
    document.querySelectorAll('.help-item').forEach(item => {
        item.addEventListener('click', () => {
            selectedHelpCategory = item.getAttribute('data-category');
            const label = document.getElementById('complaint-category-label');
            if(label) label.innerText = `Issue with ${selectedHelpCategory}`;
            document.getElementById('help-list-view')?.classList.add('hidden');
            document.getElementById('help-form-view')?.classList.remove('hidden');
        });
    });

    document.getElementById('btn-back-help-list')?.addEventListener('click', resetHelpViews);

    document.getElementById('btn-submit-complaint')?.addEventListener('click', () => {
        const text = document.getElementById('complaint-text').value;
        if(!text) return showToast('Please describe your issue.', 'error');
        
        fetch('/api/user/complaints/submit', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ category: selectedHelpCategory, description: text })
        }).then(res => res.json()).then(data => {
            if(data.success) {
                document.getElementById('help-form-view')?.classList.add('hidden');
                document.getElementById('help-success-view')?.classList.remove('hidden');
                const tId = document.getElementById('display-ticket-id');
                if(tId) tId.innerText = `#${data.ticket_id}`;
                loadSupportTickets();
            } else {
